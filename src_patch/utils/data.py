...
def get_charge_channel(density):
...
def compute_electron_count(density, cell):
    charge_density = get_charge_channel(density)
    voxel_volume = abs(np.linalg.det(cell)) / np.prod(charge_density.shape)
    return float(np.sum(charge_density) * voxel_volume)


def normalize_density_to_electron_count(density, cell, target_electron_count):
    charge_density = get_charge_channel(density)
    current_electron_count = compute_electron_count(charge_density, cell)
    if abs(current_electron_count) < 1e-12:
        raise ValueError("Cannot normalize a density with zero integrated electron count.")

    scale = float(target_electron_count) / current_electron_count
    return np.asarray(density, dtype=np.float64) * scale, float(scale), current_electron_count


def get_density_normalization_metadata(atoms, density=None):
    target_electrons = atoms.info.get("charge3net_target_electrons")
    raw_electron_count = atoms.info.get("charge3net_total_electrons")
    normalization_scale = atoms.info.get("charge3net_normalization_scale")

    if target_electrons is None or raw_electron_count is None or normalization_scale is None:
        if density is None:
            raise KeyError(
                "Density normalization metadata is missing from atoms.info and no density was provided to recompute it."
            )
        raw_electron_count = compute_electron_count(density, atoms.get_cell())
        target_electrons = int(round(raw_electron_count))
        if abs(raw_electron_count) < 1e-12:
            normalization_scale = 1.0
        else:
            normalization_scale = float(target_electrons) / float(raw_electron_count)

        atoms.info["charge3net_total_electrons"] = float(raw_electron_count)
        atoms.info["charge3net_target_electrons"] = int(target_electrons)
        atoms.info["charge3net_normalization_scale"] = float(normalization_scale)

    return {
        "target_electrons": float(target_electrons),
        "raw_electron_count": float(raw_electron_count),
