...
def resolve_master_port(default_port: int = 8888) -> str:
    ...
    if job_id and job_id.isdigit():
        ...
        if array_task_id and array_task_id.isdigit():
            unique_id += int(array_task_id)
        step_id = os.environ.get("SLURM_STEP_ID")
        if step_id and step_id.isdigit():
            unique_id += int(step_id)
        return str(10000 + (unique_id % 50000))
...
def test_from_config(cfg):
    ...
    env = {
        ...
        "master_port": resolve_master_port(),
