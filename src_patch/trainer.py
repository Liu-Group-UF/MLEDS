...
class Trainer:
    ...
    def _load_checkpoint(self, checkpoint_path):
        ...
        if "pytorch-lightning_version" in checkpoint:
            return self._load_checkpoint_legacy(checkpoint)
    ...
    def _load_checkpoint_legacy(self, checkpoint):
        ...
        if self.optimizer is not None and "optimizer_states" in checkpoint:
            self.optimizer.load_state_dict(checkpoint["optimizer_states"][0])

        if self.scheduler is not None and "lr_schedulers" in checkpoint:
    ...
    @torch.no_grad()
    def test(self, test_dl, cube_dir=None, max_predict_batch_probes=2500, defer_cube_materialization=False):
        self.model.eval()
        ...
        if cube_dir is not None:
            ...
            if not defer_cube_materialization:
                cube_dir.mkdir(exist_ok=True, parents=True)
            tmp_cube_dir = cube_dir.parent / "cube_shards" if defer_cube_materialization else self.log_dir / ".tmp_cubes"
        ...
        for i, batch in enumerate(test_dl):
            ...
            if not predictions["partial"] and not defer_cube_materialization:
        ...
        if self.local_rank == 0 and self.global_rank == 0:
            ...
            if cube_dir is not None and not defer_cube_materialization:
