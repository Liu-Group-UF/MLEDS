...
import numpy as np
import pickle
from src.utils.data import compute_electron_count
...
def convert(chgcar_file, output_density_file, output_atoms_file, filelist_file, overwrite=False, spin=False):
    ...
    total_electrons = compute_electron_count(density, atoms.get_cell())
    atoms.info["charge3net_total_electrons"] = float(total_electrons)
    atoms.info["charge3net_target_electrons"] = int(round(total_electrons))
    atoms.info["charge3net_normalization_scale"] = float(
        atoms.info["charge3net_target_electrons"] / total_electrons
    )
