...
from src.utils.data import (
    get_density_normalization_metadata,
    load_atoms_file,
    load_density_file,
    normalize_density_to_electron_count,
)
...
def deepdft_to_chgcar(density_file, atoms_file, aug_chgcar_file=None) -> VaspChargeDensity:
    ...
    metadata = get_density_normalization_metadata(atoms, density)
    target_electrons = metadata["target_electrons"]
    if target_electrons is not None:
        density, _, _ = normalize_density_to_electron_count(
            density,
            atoms.get_cell(),
            target_electrons,
        )
