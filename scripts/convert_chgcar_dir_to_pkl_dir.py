...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Optional text file containing one CHGCAR path per line to convert",
)
...
def convert(f, input_dir=None, output_dir=None, write_filelist=True, write_spin=False):
    ...
    fname = "_".join(relative_path_parts)
...
if __name__ == "__main__":
    ...
    # find all chgcar files in directory unless an explicit input list is provided
    # (e.g. to (re)convert a specific subset of structures)
    if args.input_list:
        ...
        chgcar_files = list(perturbed_structures_dir.rglob("**/CHGCAR"))
